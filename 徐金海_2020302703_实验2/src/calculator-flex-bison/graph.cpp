...
// 单节点产生图片
Agnode_t *genOneGraphNode(Agraph_t *g, struct ast_node *astnodeleft, const std::string op_str)
{
    // 先遍历左侧再遍历右侧抽象语法树
    Agnode_t *left = graph_visit_ast_node(g, astnodeleft);
...
Agnode_t *genBlockGraphNode(Agraph_t *g, struct ast_node *astnode)
{
    ...
    if (node != nullptr) {
        ...
        // 设置图形节点的属性
        agsafeset(node, (char *)"label", (char *)"block", (char *)"");agsafeset(node, (char *)"label", (char *)"block", (char *)"");
        agsafeset(node, (char *)"shape", (char *)"ellipse", (char *)"");
